...
export const XCom = () => {
  ...
  const [selectedXCom, setSelectedXCom] = useState<XComResponse | null>(null);
  ...
  const { isPending: isDeleting, mutate: deleteXCom } = useMutation({
    mutationFn: (data: DeleteXcomEntryData) => XcomService.deleteXcomEntry(data),
    onError: () => {
      toaster.create({
  ...
  const onDelete = (xcom: XComResponse) => {
    setSelectedXCom(xcom);
    onOpenDelete();
  };

  const onEdit = (xcom: XComResponse) => {
    setSelectedXCom(xcom);
    onOpenEdit();
  };
