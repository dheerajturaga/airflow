...
const EditXComModal = ({
  ...
  const { isPending, mutate } = useXcomServiceUpdateXcomEntry({
    onError: () => {
      toaster.create({
        description: translate("xcom.edit.error"),
        title: translate("xcom.edit.errorTitle"),
        type: "error",
      });
    },
    onSuccess: () => {
      onClose();
      toaster.create({
        description: translate("xcom.edit.success"),
        title: translate("xcom.edit.successTitle"),
        type: "success",
      });
    },
  });
  ...
  return (
    <Dialog.Root lazyMount onOpenChange={onClose} open={isOpen} size="lg">
      <Dialog.Content backdrop>
        <Dialog.Header>
          <Heading size="lg">{translate("xcom.edit.title")}</Heading>
        </Dialog.Header>
        <Dialog.CloseTrigger />
        <Dialog.Body>
          {isLoading ? (
            <ProgressBar isIndeterminate size="xs" />
          ) : (
            <VStack gap={4}>
              <Box width="100%">
                <Text fontWeight="bold" mb={2}>
                  {translate("xcom.key")}: {xcomKey}
                </Text>
              </Box>
              <Box width="100%">
                <Text fontWeight="bold" mb={2}>
                  {translate("xcom.value")}
                </Text>
