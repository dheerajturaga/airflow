...
const AddXComModal = ({
  ...
  const { isPending, mutate } = useXcomServiceCreateXcomEntry({
    onError: () => {
      toaster.create({
        description: translate("xcom.add.error"),
        title: translate("xcom.add.errorTitle"),
        type: "error",
      });
    },
    onSuccess: () => {
      ...
      setValue("");
      toaster.create({
        description: translate("xcom.add.success"),
        title: translate("xcom.add.successTitle"),
        type: "success",
      });
    },
  });
  ...
  return (
    <Dialog.Root lazyMount onOpenChange={onClose} open={isOpen} size="lg">
      <Dialog.Content backdrop>
        <Dialog.Header>
          <Heading size="lg">{translate("xcom.add.title")}</Heading>
        </Dialog.Header>
        <Dialog.CloseTrigger />
        <Dialog.Body>
          <VStack gap={4}>
